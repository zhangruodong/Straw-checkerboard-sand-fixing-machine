...
> ⚠️ **安全提示**：本项目驱动机器人运动部件（履带行走 + 推杆），是**控制思路的参考实现**，不是可直接上机运行的产品固件。上电前请先读 [SAFETY.md](SAFETY.md)——内含上电检查清单、本项目的已知危险点（**尤其：无物理急停按钮、自动铺草期间无通信超时保护**）及免责声明。
...
| `System/` | `Delay` 延时模块 |
| `Project.uvprojx`、`README.md`、`SAFETY.md` | 工程文件与文档 |
